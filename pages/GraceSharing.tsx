...
const GraceSharing: React.FC = () => {
  ...
  const [posts, setPosts] = useState<Post[]>(() => {
    const saved = localStorage.getItem('sgch_posts');
    return saved ? JSON.parse(saved) : [
      {
        id: 1,
        author: '김은혜 성도',
        date: '2023년 10월 27일 • 오전 06:30',
        content: '오늘 새벽 기도를 통해 주신 마음을 나눕니다. 범사에 감사하는 마음이 얼마나 큰 기쁨인지 다시금 깨닫는 시간이었습니다. 작은 일에도 하나님의 손길이 닿아 있음을 고백합니다. 성도님들 모두 평안한 하루 되시길 기도합니다.',
        likes: 24,
        comments: 5,
      },
      {
        id: 2,
        author: '이요한 집사',
        date: '2023년 10월 26일 • 오후 09:15',
        content: '오늘 목장 모임에서 나눈 말씀 시편 23편이 계속 가슴에 남네요. "여호와는 나의 목자시니 내게 부족함이 없으리로다." 삶의 파도 속에서도 그분이 계시기에 평안함을 누립니다.',
        likes: 12,
        comments: 2,
      },
    ];
  ...
  useEffect(() => {
    localStorage.setItem('sgch_posts', JSON.stringify(posts));
  }, [posts]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newPost: Post = {
      id: Date.now(),
      author: `${authorName || '익명의 성도'} ${authorTitle}`,
      date: dateStr,
    ...
    setNewPostContent('');
    localStorage.removeItem('sgch_grace_draft'); // 등록 성공 시 임시 저장 삭제
  ...
  const adminDeletePost = (id: number) => {
